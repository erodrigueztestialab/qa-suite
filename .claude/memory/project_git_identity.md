...
**Nota (2026-09-23): el repo vive en mas de una maquina, la ruta local varia.** El usuario clono el mismo repo (`erodrigueztestialab/qa-suite`) en una segunda maquina bajo `C:\Repos\qa-suite` (no `C:\TestiAlab\QA-Suite`) -- la identidad/regla de arriba aplica al REPO por su remoto, no a una ruta especifica. En esa segunda maquina no habia NINGUN `git config` (ni local ni global, `~/.gitconfig` ni siquiera existia) -- se seteo `--local` igual que en la maquina original, mismo comando. **Tambien se confirmo que `gh` CLI NO esta instalado en esa segunda maquina** (a diferencia de la original, donde ya estaba autenticado como se documenta arriba) -- los PRs ahi se abren navegando a `https://github.com/erodrigueztestialab/qa-suite/compare/develop...feature/<rama>?expand=1` en el navegador real (via claude-in-chrome) en vez de `gh pr create`. Antes de asumir que `gh` esta disponible en cualquier maquina, correr `gh --version` (o `Get-Command gh` en PowerShell) primero; si no esta, usar el flujo de navegador de arriba, que ya funciono bien 2 veces (PRs #7 y #8).
